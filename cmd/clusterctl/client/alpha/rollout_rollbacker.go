...
func (r *rollout) ObjectRollbacker(proxy cluster.Proxy, ref corev1.ObjectReference, toRevision int64, force bool) error {
	switch ref.Kind {
	case MachineDeployment:
		...
		if err := rollbackMachineDeployment(proxy, deployment, toRevision, force); err != nil {
			return err
		}
	default:
...
func rollbackMachineDeployment(proxy cluster.Proxy, md *clusterv1.MachineDeployment, toRevision int64, force bool) error {
	log := logf.Log
