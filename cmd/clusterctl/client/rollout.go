...
func (c *clusterctlClient) RolloutHistory(options RolloutHistoryOptions) error {
	clusterClient, err := c.clusterClientFactory(ClusterClientFactoryInput{Kubeconfig: options.Kubeconfig})
	if err != nil {
		return err
	}
	...
	for _, ref := range objRefs {
		if err := c.alphaClient.Rollout().ObjectViewer(clusterClient.Proxy(), ref, options.Revision); err != nil {
			return err
		}
	}
	return nil
